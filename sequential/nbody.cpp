...
//compute force between two particles (returns vector)
void compute_force_between(const simulation& s, size_t i, size_t j, double& fx, double& fy, double& fz) {
...
int main(int argc, char* argv[]) {
  ...
  for (size_t step = 0; step < nbstep; ++step) {
    ...
    // Parallel force computation per particle with **local accumulation**
    loop.parfor(0, s.nbpart, [&](size_t i){
      double fx_i=0, fy_i=0, fz_i=0;
      for(size_t j=0; j<s.nbpart; ++j){
        if(i==j) continue;
        compute_force_between(s, i, j, fx_i, fy_i, fz_i);
      }
      s.fx[i] = fx_i;
      s.fy[i] = fy_i;
      s.fz[i] = fz_i;
    ...
    // Parallel update of velocities and positions
    loop.parfor(0, s.nbpart, [&](size_t i){
      apply_force(s, i, dt);
